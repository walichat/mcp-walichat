# MCP WaliChat WhatsApp API Connector [![NPM Version](https://img.shields.io/npm/v/mcp-walichat.svg)](https://www.npmjs.com/package/mcp-walichat)
...
> ⚠️ **Note**: WaliChat MCP interface is currently in beta.
...
* To change which host `mcp-walichat` registers as the OAuth callback URL (by default `localhost`), add the `--host` flag.
...
```bash
npx mcp-walichat https://example.remote/server --transport sse-only
```
...
MCP Remote supports providing static OAuth client metadata instead of using the mcp-walichat defaults.
...
```bash
npx mcp-walichat https://example.remote/server --static-oauth-client-metadata '{ "scope": "space separated scopes" }'
# uses node readfile, so you probably want to use absolute paths if you're not sure what the cwd is
npx mcp-walichat https://example.remote/server --static-oauth-client-metadata '@/Users/username/Library/Application Support/Claude/oauth_client_metadata.json'
```
...
npx mcp-walichat https://example.remote/server --static-oauth-client-info "{ \"client_id\": \"$MCP_REMOTE_CLIENT_ID\", \"client_secret\": \"$MCP_REMOTE_CLIENT_SECRET\" }"
# uses node readfile, so you probably want to use absolute paths if you're not sure what the cwd is
npx mcp-walichat https://example.remote/server --static-oauth-client-info '@/Users/username/Library/Application Support/Claude/oauth_client_info.json'
```
...
```shell
npx -p mcp-walichat@latest mcp-walichat-client https://remote.mcp.server/sse
```
